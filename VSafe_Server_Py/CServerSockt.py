...
class CServerSocket():
    ...
    def __UpdataDatabase__(self, s, msg):
        ...
        for i in range(result[1]):
            database = (result[0][i][1]).decode('utf8')
    ...
    def __UpdataQuery__(self, s, msg):
        ...
        # 查询
        result = self.coon.query("SELECT * from `database` where `md5` = %s", (strmd5,))
    ...
    # 函数字典
    dictFun = {
        EnumMessageType.VIRUSINFO.value: __UpdataDatabase__,         #病毒库
        EnumMessageType.WHITE.value: __UpdataDatabase__,            #白名单
        EnumMessageType.BLACK.value: __UpdataDatabase__,            #黑名单
        EnumMessageType.QUERY.value: __UpdataQuery__,               # 查询
        EnumMessageType.SUBMIT.value: __UpdataSUBMIT__,             # 提交
    }
